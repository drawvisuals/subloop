...
export function FilterDropdown({
  ...
  return (
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      {isOpen && (
        <div className="absolute top-full mt-1 w-full bg-neutral-50 border border-neutral-200 rounded-lg shadow-lg z-50 py-1">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
              ...
              className={`w-full text-left px-4 py-2 text-sm text-white hover:bg-neutral-100 transition-colors ${
